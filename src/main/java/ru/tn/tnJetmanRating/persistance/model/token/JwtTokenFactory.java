...
public class JwtTokenFactory {
    ...
    public AccessJwtToken createAccessJwtToken(AuthUser authUser) {
    ...
    public JwtToken createRefreshToken(AuthUser authUser) {
        if (StringUtils.isBlank(authUser.getUsername())) {
            throw new IllegalArgumentException("Cannot create JWT Token without username");
        }
        ...
        String token = Jwts.builder()
                ...
                .setIssuedAt(Date.from(currentTime.atZone(ZoneId.systemDefault()).toInstant()))
                .setExpiration(Date.from(currentTime
                        .plusMinutes(settings.getTokenRefreshExpirationTime())
                        .atZone(ZoneId.systemDefault()).toInstant()))
                .signWith(SignatureAlgorithm.HS512, settings.getTokenSigningKey())
                .compact();
